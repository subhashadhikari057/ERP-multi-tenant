...
export default function HeroSection() {
  ...
  useEffect(() => {
    const updateRadius = () => setRadius(window.innerWidth < 768 ? 110 : 170);
    updateRadius();
  ...
  return (
    ...
    >
      {/* ------------------------------------------------------------------
         Spotlight background is now provided globally by <Spotlight />
         mounted in MainLayout, so we removed the local spotlight <div>.
      ------------------------------------------------------------------- */}
      ...
      <div
        className="
          ...
          min-h-screen pt-20
          gap-y-20 lg:gap-y-0 lg:gap-x-28
        "
      >
        {/* ---------- LEFT TEXT COLUMN ---------- */}
      ...
      <style>{`
        ...
        .animate-spin-slow-reverse  { animation:spin-slow-reverse 28s linear infinite; }
      `}</style>
    </section>
  );
}
